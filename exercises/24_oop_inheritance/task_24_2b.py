...
class MyNetmiko(CiscoIosSSH):
    ...
    def send_confid_set(self,config_commands,**kwargs):
        for cmd in config_commands:
            output=super().send_config_set(cmd,**kwargs)
    ...
    def _check_error_in_command(self,command,com_output):
        if '%' in com_output:
            error=re.search(r'% (?P<err>.+)',com_output).group()
...
if __name__ == '__main__':
    ...
    com=['loging buffered 20010']

    r1 = MyNetmiko(**device_params)
    print(r1.send_config_set(com))
